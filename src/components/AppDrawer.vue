...
<template>
  <div class="drawer">
    <div class="drawer__container" v-auto-animate>
      ...
      <div v-show="cart.length > 0 & !orderSent" class="drawer__items" :style="orderSending ? 'overflow:hidden' : 'overflow:auto'" v-auto-animate>
        <CartList />
      ...
      <button v-if="cart.length > 0" @click="makeOrder" :disabled="orderSending" class="drawer__order-button">
        Оформить заказ 
      ...
      <InfoBlock :img-url="'/img/package-icon.png'" :title="'Корзина пустая'" :subtitle="'Добавьте хотя бы одну пару кроссовок, чтобы сделать заказ.'" class="drawer__info" v-if="cart.length == 0 & !orderSent" />
      <InfoBlock :img-url="'/img/order-success-icon.png'" :title="'Заказ оформлен!'" :green-title="true" :subtitle="`Ваш заказ #${orderNum} скоро будет передан курьерской доставке`" class="drawer__info _order" v-else-if="cart.length == 0 & orderSent" />
    </div>
  </div>
</template>
