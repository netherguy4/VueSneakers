...
<template>
  <li class="order">
    ...
    <ul class="order__container">
      <OrdersListItem class="order__item" v-for="item in props.items" :key="item.id" :id="item.id" :title="item.title" :price="item.price" :image-url="item.imageUrl" />
    </ul>
    <div class="order__total">Всего: <b>{{ numberWithSpaces(totalPrice) }}</b> грн.</div>
  </li>
</template>
